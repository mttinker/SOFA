...
author: "SOFA created for U.S. Geological Survey and Seatle Aquarium by M.T. Tinker, Nhydra Ecological"
output:
  html_document: 
    fig_width: 9
editor_options:
...
required_Packages<-c("rstan","ggplot2", "gridExtra", "bayesplot", "dplyr", "htmlTable","gtools","knitr")
...
In addition to calculating the total time allocated to each prey in each bout, we can also calculate the biomass consumed for each prey type: this is accomplished by converting the linear size of each prey item (*SZ*~j~, in mm) to biomass. Linear size is estimated by observers as the maximum linear dimension relative to a paw width (excluding appendages), and this value is converted into an estimate of edible biomass using empirically derived log-log functional relationships between linear dimension and edible biomass for each prey type. If we then sum the estimated edible biomass for all recorded items of prey type *j* observed in a bout (or a portion of a bout), and divide by the total number of minutes of a bout allocated to prey type *j*in that bout (or a portion of a bout), we obtain the observed consumption rate for that prey type (*CRobs*~j,i~), in g/min.  For each observed bout we can calculate *TM*~j,i~ and *CRobs*~j,i~, as well as the mean values of the other statistics for each prey type (*HTmn*~j,i~ and *SZmn*~j,i~ ).  

We next define a set of expected parameters that correspond to these observed variables, based on a simple process model.  For prey type *j* (and excluding the UNID class), we let $\eta$ represent the TRUE proportional allocation of foraging effort (i.e. if all prey were positively identified), such that:
  $$\sum_{j=1}^{J} \eta_j=1$$
  
For each prey type *j* we specify a parameter $\omega_j$ as the probability that an item of that prey type will be positively identified. We calculate $\omega_j$ based on the distributions of the log of handling time and the log of mean prey size of prey type *j*, and the joint overlap of these distributions  with the same distribution for the UNID prey class (measured using the Bhattacharyya coefficient). The rationale is that the more similar the density distributions of these attributes are between UNID and prey type *j*, the more likely it is that *j* contributes to the UNID prey class. To account for unidentified prey in our observed data set, we define the parameter $\alpha$ as the mean proportional effort allocation to each prey type INCLUDING the UNID prey class.  For positively identified prey types:
$${{\alpha }_{j}}={{\eta }_{j}}\cdot {{\omega }_{j}}\cdot \tau $$
while for the unidentified prey class (UNID):
$${{\alpha }_{u}}=\sum\limits_{j}{{{\eta }_{j}}\cdot \left( 1-{{\omega }_{j}} \right)\cdot \tau }$$
The parameter $\tau$ represents a fitted precision parameter, which allows us to use $\alpha_j$ as the base parameters for a Dirichlet-Multinomial distribution that defines the relative probabilities of a prey type being observed in a given bout:
$$\left[ {{\theta }_{j,i}} \right] \sim Dirichlet\left( {{\alpha }_{1}},{{\alpha }_{2}},\ ...\ {{\alpha }_{J}},{{\alpha }_{U}} \right)$$
and we can relate these expected probabilities to the observed distribution of minutes allocated to each prey type on a given bout using a multinomial distribution:
$$\left[ T{{M}_{j,i}} \right]\sim Multinomial\left( \left[ {{\theta }_{j,i}} \right] \right)$$

We define parameter $\mu_{s,j}$ as the mean log size (mm) for each prey type.  We assume that observed mean prey size for prey type *j* is described by a log-normal distribution:
$$SZm{{n}_{j,i}}\sim\ lognormal\left( {{\mu }_{s,j}},{{\sigma }_{s,j}} \right)$$
where $\sigma_{s,j}$ is a parameter describing the variance in the mean size of prey *j* across bouts.  For handling time and consumption rate, we note that both of these parameters is correlated strongly with prey size: specifically, there is an approximately linear relationship between the log of each variable and the log of mean prey size.  We therefore define expected log handling time ($\mu_{h,j}$) and log consumption rate ($\mu_{c,j}$) as derived parameters:
$${{\mu }_{h,j}}={{\psi }_{1,j}}+{{\psi }_{2,j}}\cdot \log (SZmn)$$
$${{\mu }_{c,j}}={{\phi }_{1,j}}+{{\phi }_{2,j}}\cdot \log (SZmn)$$
where the fitted parameters $phi_{1,j}$, $phi_{2,j}$, $psi_{1,j}$, and $psi_{2,j}$, together describe the functional relationships between handling time, consumption rate, and prey size for each prey type.  We then assume that observed mean handling time for prey type *j* and mean consumption rate for prey type *j* are both described by log-normal distributions:
$$HTm{{n}_{j,i}}\sim\ lognormal\left( {{\mu }_{h,j}},{{\sigma }_{h,j}} \right)$$
$$CRob{{s}_{j,i}}\sim\ lognormal\left( {{\mu }_{c,j}},{{\sigma }_{c,j}} \right)$$

In addition to the base parameters of the process model, we also calculated several derived parameters.  The rate of energy intake … 
Table 1 provides a summary of all base parameters and derived parameters estimated by the model.

The observed statistics from foraging bouts were used to constrain the possible values of the base parameters of the process model.  We used standard Markov-Chain Monte Carlo methods to fit the model to the foraging data, with uninformative priors for all model parameters.  We evaluated model convergence by graphically examining chain mixing and ensuring that the Rhat statistic was close to 1 for all estimated parameters. We evaluated model fit using graphical posterior predictive checks, ensuring that the distributions of out-of-sample predictions were consistent with observed data. We present summaries of posterior distributions for both base parameters and derived parameters such as energy intake rate.
...
| ${{\tau }_{g}}$* | Precision (consistency) in diet composition across groups |
...
Note: parameters with * symbols only estimated if by-groups were incorporated in analysis
...
```{r Fig7, echo=FALSE, warning=FALSE, eval=Grp_TF, fig.cap = "Figure 7. Caterpiller plots showing posterior distributions for $\\pi$~*g,j*~, the proportion of diet (biomass consumed) made up of prey type *j* for group *g* "}
  ...
  for(g in 1:length(GL)){
    ...
    plts[[g]] = suppressMessages(mcmc_intervals(post,
               pars = paste0("PDg[",gl,",",seq(1,(NPtypes-1)),"]"),
               prob = 0.8) + 
      scale_y_discrete(name="Diet Type",labels=dfPtp$Description) +
...
ii = c(which(startsWith(vns,"PidG[")))
