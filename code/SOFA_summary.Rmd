...
We assume that the observed dives success rates specific to each prey type (*PSD*~j~), logit-transformed, are described by a normal distribution: 
...
```{r Fig2b, echo=FALSE, warning=FALSE, fig.cap = "Figure 2b. Boxplot showing posterior distributions for diet composition by Prey Class"}

plot_title = "Proportional contribution to diet (by consumed biomass) by Prey Class"
plt_Cl_pi = ggplot(df_Pclass,aes(x=Prey_Class,y=Pi)) +
		geom_boxplot(fill="grey") +
		scale_x_discrete(name="Prey Class",labels=rev(Pclass_stats$Description),
...
```{r Fig7, fig.height=GrpFgHt, echo=FALSE, warning=FALSE, eval=Grp_TF, fig.cap = "Figure 7. Caterpiller plots showing posterior distributions for $\\pi$~*g,j*~, the proportion of diet (biomass consumed) made up of prey type *j* for group *g* "}
  ...
  if(params$GL5>0){GL = c(GL,params$GL5) }
  if(params$GL6>0){GL = c(GL,params$GL6) }
  if(params$GL7>0){GL = c(GL,params$GL7) }
  if(params$GL8>0){GL = c(GL,params$GL8) }
  if(params$GL9>0){GL = c(GL,params$GL9) }
  txt1 = paste0("Proportional contribution to diet (by biomass), by ",Grpvartxt,
                      ", Forage data ",Projectname)
  plot_title = c(txt1,"","",txt1,"","",txt1,"","")
  plts = list()
  ...
  grid.arrange(grobs=plts,ncol=min(3,length(GL)))

```

```{r Fig8, fig.height=GrpFgHt, echo=FALSE, warning=FALSE, eval=Grp_TF, fig.cap = "Figure 8. Caterpiller plot showing posterior distributions for *er*~*g,j*~, the rate of energy intake while feeding on each prey type *j* for group *g* "}
  ...
  txt1 = paste0("Rate of Energy Intake (kcal/min) by prey type, by ",Grpvartxt,
                      ", Forage data ",Projectname)
...
kable(paramstats, row.names = FALSE, align = "l",
      caption = "**Table 3.** Parameter estimates for consumption rate (${{\\bar{CR}}}$, g/min), rate of energy intake (${{\\bar{ER}}}$, kcal/min), and dive success rate ($\\bar{\\lambda}$) for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 8.** Parameter estimates for $\\lambda$~*j*~, mean dive success rate by prey type, for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 9.** Estimates for $\\phi$~*1,j*~, intercept parameter for function relating log consumption rate to log size, by prey type, for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 10.** Estimates for $\\phi$~*2,j*~, slope parameter for function relating log consumption rate to log size, by prey type, for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 11.** Estimates for $\\psi$~*1,j*~, intercept parameter for function relating log handling time to log size, by prey type, for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 12.** Estimates for $\\psi$~*2,j*~, slope parameter for function relating log handling time to log size, by prey type, for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 13.** Estimates for $\\eta$~*j*~, proportional allocation of effort to prey type j, for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 14.** Estimates for $\\pi$~*j*~, proportion of diet (consumed biomass) consisting of prey type j, for the overall data set")
...
kable(Pclass_stats[,-2], row.names = FALSE,
      caption = "**Table 14b.** Estimates for proportion of diet (consumed biomass) by prey Class (standardized definitions), for the overall data set")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 15.** Estimates for $\\omega$~*j*~, probability that prey type *j* is positively identified (and thus not recorded as 'Un-ID' prey)")
...
paramstats <- paramstats %>% mutate_if(is.numeric, signif, digits = 4)
kable(paramstats, row.names = FALSE, align = "l",
      caption = "**Table 17.** Parameter estimates for consumption rate (*CR*~*g*~, g/min), rate of energy intake (*ER*~*g*~, kcal/min) and dive success rate ($\\lambda$) for each group level *g*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 18.** Parameter estimates for mean size, *S*~*g,j*~, by group level *g* and by prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 19.** Parameter estimates for mean handling time, *H*~*g,j*~, by group level *g* and by prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 20.** Parameter estimates for mean consumption rate *cr*~*g,j*~, by group level *g* and by prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 21.** Parameter estimates for mean energy intake rate *er*~*g,j*~, (kcal/min), by group level *g* and by prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 22.** Parameter estimates for $\\lambda$~*g,j*~, mean dive success rate by group level *g* and by prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 23.** Estimates for $\\phi$~*1,g,j*~, intercept parameter for function relating log consumption rate to log size, for each group *g* and prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 24.** Estimates for $\\psi$~*1,g,j*~, intercept parameter for function relating log handling time to log size, for each group *g* and prey type *j*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 25.** Estimates for $\\eta$~*g,j*~, proportional allocation of effort to prey type j, for each group level *g*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 26.** Estimates for $\\pi$~*g,j*~, proportion of diet (consumed biomass) consisting of prey type j, for each group level *g*")
...
paramstats = cbind(paramstats,sumstats[ii,c(1,3,4,8,9,10)])
kable(paramstats, row.names = FALSE,
      caption = "**Table 27.** Estimates for $\\omega$~*g,j*~, probability that prey type *j* is positively identified (and thus not recorded as 'Un-ID' prey) for each group level *g* ")
